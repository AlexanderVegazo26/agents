...
Capacity has a real cost — weigh headroom against actual growth confidence; massive over-provisioning "just in case" is a FinOps problem, not a safety margin (see `sdlc-suite:cost-optimization`).

## Monitoring against the plan

A capacity plan is only useful if actual usage is tracked against it — `sdlc-suite:site-reliability` should be watching the trend line, not just current-moment utilization, so drift from the plan is caught early.

## The feedback loop is bidirectional

`sdlc-suite:performance-engineer`'s pre-release load/scalability campaigns produce the initial capacity model; `sdlc-suite:site-reliability`'s real production data should calibrate and correct that model going forward, not just consume it once. A capacity plan that's never updated against actual observed usage drifts from reality exactly when it matters most — right before the limit it predicted is reached.
