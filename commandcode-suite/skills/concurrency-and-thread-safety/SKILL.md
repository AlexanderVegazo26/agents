...
- **Accidental sequential execution** — `await` inside a loop, one iteration at a time, when the operations were intended to run concurrently. This isn't a correctness bug, but it's a common and quiet performance one (see `sdlc-suite:debugging-methodology` for how to actually localize why something intended to be concurrent isn't).
...
- When a race is suspected but not yet reproduced, don't fix speculatively — increase the odds of reproduction first (more concurrent load, artificially widened timing windows, forced interleaving if tooling allows), confirm via `sdlc-suite:debugging-methodology`'s hypothesis-and-falsify discipline, then fix.
