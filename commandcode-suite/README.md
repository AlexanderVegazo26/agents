...
A full software-development-lifecycle framework for **Command Code** — a fixed set of agents, skills and scripted workflows, and a shared per-project memory root. Exact counts are below, generated from the tree; this paragraph does not restate them.
...
├── skills/          skills (.commandcode/skills/<name>/SKILL.md format)
├── commands/        workflow launchers (commands/*.md)
├── workflows/       standalone Node orchestration scripts + _runner.js
...
The registry is deliberately wired so **no skill is orphaned** — every skill in the table above is named by at least one agent body at its point of use. Skills do not reliably auto-trigger inside a subagent, so an unnamed skill is effectively unreachable. That property is checked by the `registry-audit` workflow.
...
One root, `.claude/memory/<project>/`, shared by every agent, with per-file ownership (see `memory-template/README.md`). Every agent body in this tree names that path, not a Command Code–specific one — the memory root is tool-agnostic by design, the same as it is in the Kimi Code port. Two rules matter more than the layout:
