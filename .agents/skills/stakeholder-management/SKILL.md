...
When two stakeholders want incompatible things, name the conflict explicitly and let the accountable decision-maker (see `governance`) resolve it — silently picking one side and not mentioning the other's ask erodes trust when it's later discovered.
