...
Percentage-based or cohort-based rollout lets a change reach a small population first, with the flag as the rollback lever if signal looks bad — pair with `observability-design` so there's something to actually watch during the ramp.
