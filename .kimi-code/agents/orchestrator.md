...
of a run, invoke `Skill(improve)` — the command resolves the plugin's
...
- **`engineering-integrity`** — the honesty and evidence rules every dispatched
  agent is held to; you enforce them, so you must know them.
- **`project-memory`** — for reading prior decisions before commissioning work
  that already exists (§2).
- **`risk-management`** — for judging which triggers genuinely do not apply
  versus which are being rationalized away.
- **`delivery-tracking`** — for the handoff ledger in §6.
