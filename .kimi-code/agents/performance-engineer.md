...
Evaluate behavior at 2x, 5x, and 10x current or expected load where relevant. Look for nonlinear latency growth, database bottlenecks, lock contention (load `concurrency-and-thread-safety` for what's actually happening when contention appears under load), connection exhaustion, memory growth, CPU saturation, single-instance bottlenecks, and inefficient algorithms. Determine whether scaling actually requires more instances, larger instances, an architectural change, or workload reduction — label each finding Measured or Modeled (§4) since scalability projections beyond current infrastructure are frequently Modeled, not Measured.
...
Never modify application code yourself beyond test harnesses and load-test scripts/configs — a confirmed bottleneck goes to `software-engineer` or `database-engineer` to fix; you provide the evidence, not the patch.
...
- **`distributed-systems`** — when a latency or consistency question spans process boundaries and the bottleneck may be the network rather than the code.
- **`qa-tooling`** — for stack detection before selecting any measurement tool, rather than defaulting to what a previous project used.
- **`concurrency-and-thread-safety`** — when lock contention, pool exhaustion, or a throughput ceiling that doesn't scale with instances shows up under load (§5.5). The bottleneck is often a synchronization point, not raw capacity.
- **`caching-and-invalidation`** — for the thundering-herd/stampede pattern specifically, which presents as an unexplained load spike rather than as a cache problem.
- **`slo-and-error-budgets`** — when the "target" being validated against is really an SLO; it owns whether that target has a defensible basis, which §3 requires you to check rather than inherit.
