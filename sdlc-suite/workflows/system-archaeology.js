...
Record what you found AND what you could not determine. If the auth mechanism cannot be located, say so explicitly: both downstream agents treat that as a stop condition.`),
      { agentType: 'sdlc-suite:product-archaeologist', label: 'detect-stack', phase: 'Detect', schema: STACK_SCHEMA },
    )
...
Stack context: ${JSON.stringify(stack.determined)}`),
          { agentType: 'sdlc-suite:persona-discovery', label: 'who', phase: 'Excavate' },
        ),
      () =>
        dispatch(
...
Stack context: ${JSON.stringify(stack.determined)}`),
          { agentType: 'sdlc-suite:product-archaeologist', label: 'what', phase: 'Excavate' },
        ),
    ])
  ...
  const crossArtifact = await runPhase('Cross-check', 'Synthesize', async () => {
    const mismatches = await dispatch(
...
AS-BUILT (what):
${asBuilt}`),
      { agentType: 'sdlc-suite:product-archaeologist', label: 'cross-check', phase: 'Cross-check' },
    )
...
PERSONAS:
${personas ?? '(none)'}`),
      { agentType: 'sdlc-suite:product-archaeologist', label: 'prd', phase: 'Synthesize', schema: PRD_SCHEMA },
    )
...
${prdText ?? '(the PRD agent produced no text)'}`),
      { agentType: 'sdlc-suite:product-archaeologist', label: 'handoff', phase: 'Synthesize' },
    )
  ...
  const prd = requireResult('prd', agentResult(synthArtifact, 'prd'))
