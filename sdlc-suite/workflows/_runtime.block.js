// Canonical copy of the runtime block shared by every workflow in this directory.
//
// A Workflow-tool script runs in a node:vm sandbox with no `require`, so this
// block cannot be imported; it is spliced into each workflow between the two
// marker lines below by `python sdlc-suite/tools/runtime_block.py --write`.
// Edit it HERE, never in a workflow. `_wiring.test.js` and
// `runtime_block.py --check` both fail on any drift.
//
// This file is not a workflow and is not runnable on its own: it references
// `WORKFLOW`, `args`, `agent` and `log`, which the host workflow provides.
