...
A full software-development-lifecycle framework for Claude Code: a fixed set of agents, skills, workflows and slash commands, and a shared per-project memory root. Exact counts are below — this file does not restate them in prose, because a typed count is the thing that goes stale silently while sounding authoritative.
...
That rule governs behavior *once an agent is invoked*. It does not make a caller invoke anything — which is why the suite now also carries an `orchestrator` (below) *and* ships [`ROUTING.md`](ROUTING.md), the platform half of the routing policy: `orchestrator`'s own dispatch table only fires once something invokes `orchestrator`, so it binds `orchestrator`, not the caller. `ROUTING.md` binds the caller — install it via `/sdlc-suite:install-routing`.
...
Currently **21 of 22 agents** hold the `Skill` tool *and* carry the `Skills loaded` output-contract line — verified by grepping `Skills loaded` across every agent file in `agents/` and confirming the one miss against its frontmatter. The one exception, `qa-runner`, is correct as-is: it holds no `Skill` grant at all, by design — a pure execution specialist with no procedural knowledge to load. This count does not reconcile arithmetically against the 13 originally fixed above; agents have been added to the roster (`orchestrator` among them) since that count was taken, so it has grown rather than merely closed.
