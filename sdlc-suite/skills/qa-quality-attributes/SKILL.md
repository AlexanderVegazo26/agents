...
**Usability** — recognizability, learnability, operability, error protection (does the UI stop likely mistakes before they happen), UI aesthetics, accessibility. Accessibility is not optional polish — see the `sdlc-suite:accessibility` skill for a WCAG-specific pass.
...
This pass is what prevents "all tests green" from meaning "we only checked the happy path." Pair with `sdlc-suite:qa-techniques` for *how* to test each in-scope characteristic, and `sdlc-suite:qa-triage` for classifying what a failure here actually means.
