...
- Confirm the same value round-trips through serialize → deserialize without drift. This is a natural metamorphic check (per `sdlc-suite:qa-engineer`'s technique set) for any datetime crossing a wire or storage boundary.
...
- Where `sdlc-suite:qa-engineer`'s boundary-value analysis is being applied to a date/time input, treat DST transition points and month/year boundaries as boundaries in the formal sense, not just the numeric min/max of the range.
