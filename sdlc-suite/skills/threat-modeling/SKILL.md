...
4. Feed findings back to `sdlc-suite:solution-architect`/`sdlc-suite:software-engineer` before implementation, not as a post-hoc audit.

This is `sdlc-suite:security-engineer`'s primary technique (§3.1 of that agent) for Tier 3 or significant new designs — the agent doing the threat model documents risk and mitigation options but doesn't accept risk on anyone else's behalf; acceptance is the owning human's or `sdlc-suite:release-manager`'s call.
