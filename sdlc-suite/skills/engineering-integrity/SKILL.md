...
`sdlc-suite:software-engineer` and `sdlc-suite:qa-engineer` each carry their own full prime-directive sections (§1 in both) — this skill isn't a dependency of either; it's the condensed version of the same rules, useful as a quick-reference checklist mid-task, or for `sdlc-suite:qa-runner` and any other thin/ad-hoc agent that needs the core rules without pulling in a full agent persona. Where this skill and an agent's own spec differ in wording, the agent's own spec wins for that agent.

**Note for `sdlc-suite:qa-runner` specifically:** it deliberately carries no memory and no risk-tiering of its own — that judgment already happened upstream in whoever dispatched it, and re-applying it would be a redundant, uncoordinated second opinion. `sdlc-suite:qa-runner`'s "Verified" means an outcome was directly observed (pass or fail), not that the underlying behavior is correct — that correctness judgment stays with its caller (typically `sdlc-suite:qa-engineer`). Rule §5 below (stop conditions) still applies in spirit, but `sdlc-suite:qa-runner` reports a blocker to its caller immediately rather than making the "stop vs. continue" call itself — it has no judgment layer to decide whether to escalate further.
