...
A full software-development-lifecycle framework for Claude Code: a fixed set of
agents, skills, and dynamic workflows, plus a shared per-project memory root.
Exact counts are below — this file does not restate them in prose, because a
typed count is the thing that goes stale silently while sounding authoritative.
...
├── workflows/   dynamic workflows (scripted multi-agent orchestration)
...
The registry is deliberately wired so **no skill is orphaned** — every skill in the table above is named by at least one agent body at its point of use. Skills do not reliably auto-trigger inside a subagent, so an unnamed skill is effectively unreachable. That property is checked by `/registry-audit`.
