...
class Run {
  constructor(opts) {
    ...
    if (resumeFrom) {
      ...
      // Containment, enforced here as well as by the caller. Security review
      // showed `../../../outside/planted` resolving to an attacker-authored run
      // whose phase artifacts were then replayed into downstream prompts with
      // the authority of "this is what your own earlier phases produced" — and
      // `RUN.dir` becoming that directory, so every later bridge script was
      // written and executed from it. An absolute path did the same.
      //
      // Two layers on purpose: the workflow's `resumeIdOf` rejects a value that
      // is not a single path segment, and this rejects anything that resolves
      // outside `runsRoot` regardless of how it got here. A run id is a name,
      // never a path.
      const dir = path.resolve(this.runsRoot, resumeFrom)
      const inside = path.relative(this.runsRoot, dir)
      if (path.isAbsolute(resumeFrom) || inside.startsWith('..') || path.isAbsolute(inside)) {
        throw new Error(
          `cannot resume ${resumeFrom}: it resolves outside ${RUNS_DIR_NAME}. ` +
          `A run id is a single directory name, not a path.`)
      }
      let manifest
      try {
      ...
      for (const ph of manifest.phases || []) {
        ...
        // `ph.artifact` is read out of the manifest file we just loaded, so on a
        // resume it is attacker-influenced input, not our own output. Security
        // review showed a manifest carrying `"artifact": "../secrets.json"`
        // reading an arbitrary JSON file into the replayed phase — and thence
        // into every downstream prompt. A legitimate artifact name never
        // contains a separator, so require that rather than trying to sanitise.
        if (typeof ph.artifact !== 'string' ||
            ph.artifact.includes('/') || ph.artifact.includes('\\') ||
            ph.artifact.includes('..') || path.isAbsolute(ph.artifact)) {
          throw new Error(
            `cannot resume ${this.runId}: phase "${ph.title}" names artifact ` +
            `${JSON.stringify(ph.artifact)}, which is not a plain filename`)
        }
        let artifact
        try {
