...
Emit specs conforming to personas-schema-template.yaml, which ships with the exploration-charter skill — load that skill to read the schema rather than constructing a path to it. Report ambiguous capability entries as the high-value output they are, not as a gap to fill by guessing. State explicitly what you could not determine.`),
      { agentType: 'persona-discovery', label: 'discover', phase: 'Discover', schema: PERSONA_SCHEMA },
    )
...
Do NOT probe anything in this persona's forbidden list — record it and leave it to boundary-prober.`),
          { agentType: 'persona-runner', label: `explore:${p.id}`, phase: 'Explore' },
        ),
      ),
    )
...
On a failed handoff, record exactly which one broke, preserve the ledger, and halt — do not skip ahead or fabricate the missing state.`),
            { agentType: 'journey-orchestrator', label: `journey:${i + 1}`, phase: 'Journeys' },
          ),
        ),
      )
...
${journeyResults.length ? journeyResults.join('\n\n---\n\n').slice(0, 6000) : '(no journeys supplied — this coverage gap belongs in the "what was NOT tested" section)'}`),
      { agentType: 'qa-engineer', label: 'triage', phase: 'Report' },
    )
